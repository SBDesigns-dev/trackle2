...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-tube") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-tube", JSON.stringify(places));
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-tube") || "{}")
  ).length;
