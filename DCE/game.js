...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-dlr") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-dlr", JSON.stringify(places));
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-dlr") || "{}")
  ).length;
...
App.prototype.resetGame = function () {
  // Clear local storage
  window.localStorage.removeItem("places-dlr");
